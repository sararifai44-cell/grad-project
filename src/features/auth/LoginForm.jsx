...
import { useState } from "react"; // إضافة useState لإدارة المدخلات
import { motion } from "framer-motion";
import { Mail, Lock, LogIn } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useLoginMutation } from "./authApiSlice"; // استيراد الـ Hook

const LoginForm = ({ onLogin }) => {
  const navigate = useNavigate();
  
  // 1. تعريف حالات الإدخال وحالة الـ API
...
const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // إرسال الطلب للباك إند وانتظار النتيجة
      await login({ username, password }).unwrap();
      
      // تشغيل دالة onLogin التي ستتكفل بتشغيل الفيديو والانتظار ثم التوجيه
      if (onLogin) onLogin(); 
      
      // ❌ قمنا بحذف navigate("/") من هنا 
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-5 font-sans text-emerald-950">
        ...
        <motion.div variants={itemVariants} className="group">
          <label className="block text-xs text-emerald-950/70 mb-1.5 font-bold">اسم المستخدم</label>
        ...
        {isError && (
          <p className="text-red-500 text-[10px] text-center font-bold">
            {error?.data?.detail || "تأكدي من صحة بيانات الدخول"}
          </p>
        )}
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className="w-full bg-andalus-clay hover:bg-[#6b340f] text-sand-100 font-andalus-kufi text-lg py-3 rounded-xl mt-5 flex justify-center items-center gap-2.5 transition-colors shadow-xl group disabled:opacity-70"
        >

...
const handleScrollToAnalyzer = () => {
  ...
  const getPlaceholder = (index, isShatr1) => {
    ...
    const example = placeholders[index % placeholders.length];
    return isShatr1 ? example.s1 : example.s2;
  };

  return (
    <div className="min-h-screen font-['Amiri',_serif] pb-24 text-right" style={{ backgroundColor: theme.bg }} dir="rtl">
      
      {/* ================= 1. قسم كيف يعمل الموقع ================= */}
   {/* ================= كيف يعمل الموقع (تصميم جديد) ================= */}
{/* ================= كيف يعمل الموقع (تصميم حي وناعم) ================= */}
<section className="py-12 px-6 lg:px-12 bg-[#f0f7f1] select-none" dir="rtl">
  <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center lg:items-center gap-8 lg:gap-12">
    ...
    <div className="w-full lg:w-1/3 flex flex-col justify-center text-center lg:text-right shrink-0">
      ...
      {/* حاوية النص (ستايل بطاقة اقتباس فخمة) */}
      <div className="bg-white rounded-2xl p-5 mb-6 max-w-md mx-auto lg:mx-0 relative shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-[#e5dcd3] border-r-[6px] border-r-[#6b4423] group hover:shadow-[0_8px_30px_rgba(107,68,35,0.08)] transition-all duration-300">
        ...
        {/* أيقونة اقتباس خفيفة في الخلفية */}
      ...
      <div className="flex justify-center lg:justify-start">
        <button 
          ...
          className="bg-[#6b4423] text-white px-6 py-3 rounded-[0.5rem] font-bold text-lg shadow-[0_8px_20px_rgba(107,68,35,0.3)] hover:bg-[#4a3427] hover:-translate-y-1 hover:shadow-[0_12px_25px_rgba(107,68,35,0.4)] transition-all duration-300 flex items-center gap-3 active:scale-95 group"
        >
    ...
    {/* ================= القسم الأيسر: الصور SVG ================= */}
    {/* تقليل الفراغات بين الأعمدة ليكون ملموم أكثر */}
    <div className="w-full lg:w-2/3 grid grid-cols-1 sm:grid-cols-3 gap-6">
      {steps.map((step, i) => (
        <div key={i} className="flex flex-col items-center group mt-4 lg:mt-0">
          ...
          {/* حاوية الـ SVG: تحولت من aspect-[4/5] الطويلة إلى aspect-square المربعة */}
          ...
          {/* ستايل الرقم واسم الخطوة */}
          <div className="flex flex-col items-center -mt-2">
            {/* الرقم */}
            <span className="text-3xl md:text-4xl font-black text-[#8b7355] opacity-40 mb-1 font-serif group-hover:opacity-80 group-hover:-translate-y-1 transition-all duration-300">
              {step.num}
            </span>
        ...
        <h3 className="text-base md:text-lg font-bold text-white 
                         ...
                         px-8 py-2.5 
                         rounded-full 
                         shadow-[0_8px_20px_rgba(107,68,35,0.3)] 
                         -mt-8 z-20 relative
                         hover:bg-[#4a3427] hover:-translate-y-1 hover:shadow-[0_12px_25px_rgba(107,68,35,0.4)] transition-all duration-300">
            {step.title}
          </h3>
          </div>
      ...
      {/* ================= 2. القسم الرئيسي: الأداة والنتائج ================= */}
      <section className="px-4 max-w-[1400px] mx-auto mb-20 bg-white/40 py-12 rounded-3xl border border-[#e5dcd3]/50">
        
        {/* العنوان الرئيسي للأداة */}
        <div className="flex items-center justify-center gap-4 mb-10">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-center" style={{ color: theme.primary }}>
            أداة تحليل الشعر العربي
          </h2>
        </div>

        {/* الحاوية الرئيسية مقسومة بنسبة 7 إلى 5 */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full items-stretch mx-auto px-2 md:px-6">
          
          {/* ---------------- القسم الأيمن: إدخال الأبيات ---------------- */}
          <div 
            className="lg:col-span-7 bg-white w-full h-[540px] rounded-2xl shadow-sm border-2 p-6 md:p-8 flex flex-col overflow-hidden"
            style={{ borderColor: theme.mutedBtn }}
          >
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar flex flex-col gap-3 mb-4">
              {verses.map((verse, index) => (
                <div key={index} className="flex flex-col md:flex-row gap-2 items-center relative group shrink-0">
                  <div className="hidden md:flex w-7 h-7 shrink-0 rounded-full bg-[#7a9b76]/15 text-[#7a9b76] items-center justify-center font-bold text-xs font-sans">
                    {index + 1}
                  </div>
                  <div className="flex-1 w-full flex flex-col gap-1">
                    <input 
                      type="text" 
                      placeholder={getPlaceholder(index, true)}
                      value={verse.shatr1}
                      onChange={(e) => handleVerseChange(index, 'shatr1', e.target.value)}
                      className="w-full bg-[#faf8f3] border border-[#b09c8a]/40 rounded-lg px-4 py-2.5 outline-none focus:border-[#6b4423] focus:bg-white transition-all text-lg text-[#3d2e1f] text-center"
                      dir="rtl"
                    />
                  </div>
                  <div className="hidden md:flex items-center justify-center text-[#b09c8a] opacity-30">
                    <Feather size={16} className="rotate-45" />
                  </div>
                  <div className="flex-1 w-full flex flex-col gap-1">
                    <input 
                      type="text" 
                      placeholder={getPlaceholder(index, false)}
                      value={verse.shatr2}
                  ...
                  <div className="w-8 flex justify-center">
                    {verses.length > 1 && (
                      <button onClick={() => handleRemoveVerse(index)} className="text-red-400 hover:text-red-600 transition-colors">
                        <Trash2 size={18} />
                      </button>
                    )}
                  </div>
            ...
            <div className="shrink-0 pt-3 border-t border-[#e5dcd3]">
              ...
              <div className="flex justify-center">
                <button 
                  ...
                  className="text-white px-10 py-3 rounded-xl font-bold text-lg flex items-center gap-2 shadow-lg w-full md:w-auto justify-center transition-all active:scale-[0.98] disabled:opacity-50"
                  ...
                  <div className="flex flex-col gap-4 animate-in fade-in duration-500">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-white border border-[#e5dcd3] border-r-4 border-r-[#b09c8a] rounded-lg p-4 flex flex-col justify-center shadow-sm relative overflow-hidden">
                        ...
                        <span className="text-[12px] text-gray-500 font-bold uppercase mb-1 font-sans">الغرض الشعري</span>
                      ...
                      <div className="bg-white border border-[#e5dcd3] border-r-4 border-r-[#d4b08c] rounded-lg p-4 flex flex-col justify-center shadow-sm relative overflow-hidden">
                        ...
                        <span className="text-[12px] text-gray-500 font-bold uppercase mb-1 font-sans">القافية</span>
                    ...
                    <div className="bg-white border border-[#e5dcd3] border-r-4 border-r-[#7a9b76] rounded-lg p-5 flex items-center gap-4 shadow-sm">
                      <div className="w-12 h-12 rounded-full bg-[#faf8f3] flex items-center justify-center text-[#7a9b76] shrink-0 border border-[#7a9b76]/20">
                        <Music size={24} />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[12px] text-gray-500 font-bold uppercase mb-1 font-sans">البحر الشعري والإيقاع</span>
                    ...
                    <div className="bg-white border border-[#e5dcd3] border-r-4 border-r-[#6b4423] rounded-lg p-5 shadow-sm flex flex-col relative">
                      ...
                      <div className="text-[#4a3427] text-lg leading-relaxed text-justify pr-1">
